...
## SSH Keypair: Skapa med ssh-keygen

SSH använder public key-kryptografi för säker autentisering. Du skapar ett nyckelpar:

- **Private key**: Hålls hemlig, finns på din klient
- **Public key**: Delas med servrar, läggs i ~/.ssh/authorized_keys
...
# Med specifik filnamn
ssh-keygen -t ed25519 -f ~/.ssh/my_server_key
```

**Frågor som ställs**:
- Fil att spara nyckeln i: ~/.ssh/id_ed25519 (default)
- Passphrase: Valfritt lösenord för att skydda private key

### Vad händer om du tappar bort passphrase?

**Viktigt**: Om du tappar bort passphrase till din privata nyckel kan du INTE återställa den. Passphrasen används för att kryptera den privata nyckeln lokalt.

```bash
# Om passphrase tappas bort:
# 1. Du kan inte dekryptera den privata nyckeln
# 2. Nyckeln blir obrukbar
# 3. Du måste skapa en ny nyckel och distribuera den igen

# Lösning: Använd password manager för att spara passphrase
# eller skapa nycklar utan passphrase (mindre säkert, men användbart för automation)
```
...
# Serverns publika nyckel (fingerprint) sparas i ~/.ssh/known_hosts
```

**Vad händer**: SSH-servern skickar sin publika nyckel för verifiering av identitet. Om du accepterar sparas den i ~/.ssh/known_hosts.

```bash
# Visa known_hosts
cat ~/.ssh/known_hosts

# Om serverns nyckel ändras (t.ex. efter reinstall):
# WARNING: REMOTE HOST IDENTIFICATION HAS CHANGED!
# Detta kan betyda att någon försöker göra man-in-the-middle attack
# eller att servern faktiskt har ändrats

# Ta bort gammal entry
...
ls -la ~/.ssh/
# id_ed25519      # Private key (HEMLIG!)
# id_ed25519.pub  # Public key (kan delas)
```

**Viktigt**: Skydda private key

```bash
# Rätt behörigheter
chmod 600 ~/.ssh/id_ed25519      # Private key: endast ägare kan läsa
chmod 644 ~/.ssh/id_ed25519.pub  # Public key: kan delas
chmod 700 ~/.ssh                 # .ssh katalog: endast ägare
```

## Distribuera med ssh-copy-id

Det enklaste sättet att kopiera din public key till en server:

```bash
# Kopiera public key till server
...
**Vad händer**: ssh-copy-id lägger till din public key i ~/.ssh/authorized_keys på servern.
...
## SSH Agent: Använda ssh-add för att slippa lösenfraser

SSH Agent håller dina private keys i minnet, så du inte behöver skriva passphrase varje gång.
...
Agent Forwarding låter din lokala SSH-agent användas för att nå servrar via en hopp-server.
...
# Nu kan du från jumpserver använda din lokala agent
...
**Användning**: När du behöver nå servrar via en hopp-server (bastion host) utan att kopiera nycklar till hopp-servern.

**Säkerhetsvarning**: Agent forwarding kan vara en säkerhetsrisk om hopp-servern komprometteras - använd bara på betrodda servrar.
...
```bash
# Starta SSH agent om den inte redan körs
...
## Security: Inaktivera lösenordsinloggning i /etc/ssh/sshd_config

När du har konfigurerat SSH-nycklar, bör du inaktivera lösenordsinloggning för extra säkerhet.

```bash
# Redigera SSH server config
sudo nano /etc/ssh/sshd_config

# Ändra dessa rader:
PasswordAuthentication no
PubkeyAuthentication yes
PermitRootLogin no  # Förhindra root-login (rekommenderat)
...
### Testa konfiguration innan restart

```bash
# Testa konfiguration (ingen restart)
sudo sshd -t
...
2. Eller via cloud provider's console
...
Skicka lokal trafik till remote server via SSH.

```bash
# Format: ssh -L [local_port]:[destination]:[destination_port] user@ssh_server
...
**Användningsfall**: Nå en databas på remote server som bara lyssnar på localhost.

```bash
# Exempel: Nå MySQL på remote server
...
Skicka remote trafik tillbaka till din lokala maskin.

```bash
# Format: ssh -R [remote_port]:[local_destination]:[local_port] user@ssh_server
ssh -R 8080:localhost:3000 user@server.com

# Nu kan någon på server.com nå din lokala port 3000 via server:8080
```

**Användningsfall**: Exponera lokal utvecklingsserver till internet via remote server.

```bash
# Exempel: Exponera lokal web server
ssh -R 8080:localhost:3000 user@server.com
# Nu kan någon nå http://server.com:8080 och se din lokala server

# Remote forwarding exponerar en lokal port till användare på fjärrservern
# Användbart för:
# - Utveckling och testning
# - Temporär exponering av lokala tjänster
# - Debugging av remote applikationer som behöver nå lokala resurser
```

**Viktigt**: Remote forwarding kräver ofta `GatewayPorts yes` i sshd_config på servern för att exponera porten externt (inte bara localhost).
...
**Skillnad från port forwarding**: SOCKS-proxy är dynamisk - den kan hantera flera destinationer, medan port forwarding är statisk (en specifik destination).

### Tunnling med bakgrundsprocess

```bash
# Kör tunnel i bakgrunden
ssh -f -N -L 8080:localhost:80 user@server.com
# -f = background
# -N = ingen remote command
...
# Ta bort gamla nyckeln från server
ssh user@server.com
...
```bash
# Kopiera fil från lokal till remote
scp file.txt user@server.com:/path/to/destination/

# Kopiera fil från remote till lokal
...
**Skillnad från rsync**: scp är enklare men rsync är mer effektivt för stora mappar (kopierar bara ändringar).

## Viktiga takeaways

- **SSH Keypair**: Private key (hemlig) + Public key (delas)
- **Passphrase**: Om den tappas bort kan nyckeln inte återställas - skapa ny nyckel
- **known_hosts**: Sparar server-fingerprints för att förhindra man-in-the-middle attacks
- **ssh-copy-id**: Enklaste sättet att distribuera public key
- **SSH Agent**: Håller nycklar i minnet, slipp skriva passphrase
- **ssh-add -l**: Lista nycklar i agenten
- **Agent Forwarding**: Använd lokal agent via hopp-server (ForwardAgent yes)
- **Säkerhet**: Inaktivera lösenordsinloggning när nycklar fungerar
- **MaxAuthTries**: Begränsa antal inloggningsförsök per anslutning
- **Local Forwarding**: `-L` = skicka lokal trafik till remote
- **Remote Forwarding**: `-R` = skicka remote trafik till lokal (exponera lokal port)
