    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            const [statsRes, activityRes, healthRes] = await Promise.all([
                fetch(`${API_BASE_URL}/api/admin-v2/stats/overview`, {
                    headers: { Authorization: `Bearer ${token}` }
                }),
                fetch(`${API_BASE_URL}/api/admin-v2/stats/activity?days=7`, {
                    headers: { Authorization: `Bearer ${token}` }
                }),
                fetch(`${API_BASE_URL}/api/admin-v2/stats/system-health`, {
                    headers: { Authorization: `Bearer ${token}` }
                })
            ])
